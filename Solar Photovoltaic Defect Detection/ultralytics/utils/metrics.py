...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, IdIoU=False, KIoE=False, PIoU=False, PIoU2=False,  eps=1e-7, scale=0.0, Lambda=1.3):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    ...
    c = cw * ch + eps  # convex area

    # IoU
    iou = inter / union
    ioe = inter / c
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or IdIoU or KIoE or PIoU or PIoU2:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or EIoU or SIoU or ShapeIoU or PIoU or PIoU2 or IdIoU or KIoE:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
                return ioc - distance - 0.5 * shape_cost
            elif PIoU or PIoU2:
                ...
                dh1 = torch.abs(b1_y2.minimum(b1_y1)-b2_y2.minimum(b2_y1))
                dh2 = torch.abs(b1_y2.maximum(b1_y1)-b2_y2.maximum(b2_y1))
                # m = ((dw1+dw2)/torch.abs(w2)+(dh1+dh2)/torch.abs(h2))

                m = rho2/(w2**2 + h2**2 + eps)
                n = (dw1*dh1 + dw2*dh2)/(w2*h2 + eps)
                m2 = rho2 / c2
                m3 = (w1/w2)+(h1/h2)
                # n2 = (dw1*dh1 + dw2*dh2)/(w1*h1 + eps)
                n3 = (dw1*dh1 + dw2*dh2)/(cw*ch + eps)
                n4 = (torch.abs(w1-w2)/cw) + (torch.abs(h1-h2)/ch)
                n5 = ((w1-w2)**2)/(cw**2) + ((h1-h2)**2)/(ch**2)
                n6 = torch.abs(w1*h1 - w2*h2)/c
                p = m2 + n
                # p2 = m + n2
                l1 = 1 - iou - 1/(1 + (3*p)**2) + 1
                # l1 = 1 - iou - torch.exp(-p ** 2) + 1
                # l1 = 1 - ioc + m2 + n5
                # l1 = 1 - ioc - 1 / (1 + (3 * m3) ** 2) + 1 + n6 + m2
                if IdIoU:
                    return 1 - l1
            ...
            return iou - rho2 / c2  # DIoU
        c_area = cw * ch + eps  # convex area
        return ioe - (c_area - union) / c_area
    return iou  # IoU
